import { View, Text, ScrollView, StyleSheet,Image,TouchableOpacity,Pressable,SafeAreaView,Linking ,Dimensions,Animated,Modal,Platform  } from "react-native";
  ...
  const { mutate: applyCreateBill, data: billData, 
    ...
    useEffect(() => {
      if(applyCheckParamsData?.data?.error_code == 1){
        const blobData = dataURLtoBlob(uri)
        // const blobData = dataURLtoBlob(faceImage)
        let file = blobData
    ...
    const getLoan = (async () => {
      if(isChecked){
        ...
        if(userStore.faceData != ''){
          console.log('useEffect face data update')
          const imageUri = userStore.faceData
