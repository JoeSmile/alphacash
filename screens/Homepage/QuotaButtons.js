...
export function QuotaButtons() {
  ...
  const [cashLoan, bill, hasBill] = useUserQuota(s => [s.cashLoan, s.bill, s.hasBill])
  ...
  if (hasBill) {
    return <View>
      ...
      {/* need modify some info */}
      {
        ...
        <View style={{
          ...
          <Image 
            ...
            />
            The payment cannot be made successfully because your account number is wrong,please modify the account number
          </Text>
        </View>
      ...
      {
        !hasError && hasBill && <View>
        <FButton title='Repay Now' onPress={() => console.log("repay now")}/>
        <FButton title='View Detail' onPress={() => console.log('go to bill detail')}/>
        </View>
      }
